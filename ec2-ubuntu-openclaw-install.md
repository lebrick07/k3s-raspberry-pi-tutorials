...
2. **Node.js 22.x**: Checks for existing Node.js ≥22.12.0; installs via NodeSource only if needed
...
6. **OpenClaw Setup**: Runs `openclaw setup --yes` non-interactively
7. **Gateway Mode**: Sets gateway mode to `local` by default (configurable via `GATEWAY_MODE` env var)
8. **Gateway Auth**: Sets recommended token-based authentication
9. **Gateway Token**: Generates gateway token using `openclaw devices token generate`
